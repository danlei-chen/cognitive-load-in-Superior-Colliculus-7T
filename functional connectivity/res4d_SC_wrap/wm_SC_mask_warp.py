...
# subj_list = os.environ.get("SUBJ_LIST").split(" ")
subj_list = [i.split('/')[-5] for i in glob.glob('/scratch/data/*/model/*/_modelestimate0/res4d.nii.gz')]
subj_list1 = [i[0:7] for i in subj_list if i[-1]=='b']
subj_list2 = [i for i in subj_list if i[-1]!='b']
subj_list = subj_list1+subj_list2
subj_list.sort()
print(subj_list)
print(len(subj_list))
subj_existed = [i.split('/')[-1].split('w_')[-1].split('__')[0] for i in glob.glob('/scratch/output/res4d/warped_files/*')]
...
for subj in subj_list:
	data_templates = {'res4d': '/scratch/data/'+subj+'/model/_*/_modelestimate0/res4d.nii.gz'}
	work_dir = '/scratch/wrkdir'
	out_dir = '/scratch/output'
	...
	warp_template = os.path.join('/scratch/dartel_flow', 'u_SC*'+subj+'*_Template_resampled.nii')
