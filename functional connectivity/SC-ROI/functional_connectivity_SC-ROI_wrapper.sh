...
setenv PROJNAME wm_conn_SC-ROI
...
chmod a+rwx /scratch/$USER/$PROJNAME/wrkdir/functional_connectivity_SC-ROI_startup.sh
cd /scratch/$USER

$SINGULARITY exec \
--bind "$DATA/warped_files:/scratch/data/res4d" \
--bind "$SCPATH/SC_mask:/scratch/data/SC_mask" \
...
--bind "$OUTPUT/SC-ROI2:/scratch/output" \
$IMAGE\
/scratch/wrkdir/functional_connectivity_SC-ROI_startup.sh
